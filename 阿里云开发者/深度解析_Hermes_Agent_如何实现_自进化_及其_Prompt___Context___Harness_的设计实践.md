...
原创 飞樰 阿里云开发者 2026-04-24 08:30 浙江

> 原文地址: [https://mp.weixin.qq.com/s/2xFei8dMx99lc-iyrZZrww](https://mp.weixin.qq.com/s/2xFei8dMx99lc-iyrZZrww)

![](https://mmbiz.qpic.cn/mmbiz_jpg/Z6bicxIx5naLXvmT74TywwBl3icdKFT8O5E3DajCvLsUYx6KACVqYu7ncC965p2ReNQNZA50KP9oic3TK3Q9Arlww/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz/j7RlD5l5q1y4zDOMXx4p5QpvbFV17tsxCudib57u1zZg7rd5DQQD3iaicLIQ6ibXssvbEtdLuS97f2FTvlzsTXmqARibwZy2PwMJDjP6867B78Xc/640?wx_fmt=other&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz/j7RlD5l5q1w3hrKz0h6Dic4KVhBdJTowHGS9I6TFpROHy6HrQWXXfz0F7t17SfISJXictauEbR4ibdj5pHxU9hNmweOyH1lyLiaJQkRxgXg6Z8Y/640?wx_fmt=other&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz/j7RlD5l5q1x3ia0bTktcibSIducINP06dhdzVYTMgVOIMpgMLEribbPicf9cHOdGGX1VyUz2XRjMTf7AUesfBs6bgE8ZS3VMKFWoGibaibdgXrRjg/640?wx_fmt=other&from=appmsg)
...
在数据预处理层面，Hermes 依赖三个核心函数来确保训练数据的质量与规范性：save\_trajectory负责将 Agent 的运行轨迹以追加模式持久化存储至 JSONL 文件中，实现数据的增量积累；convert\_scratchpad\_to\_think则专注于格式标准化，将内部使用的<REASONING\_SCRATCHPAD>标签转换为模型训练通用的<think\>格式，以适配主流大模型的思维链（CoT）训练要求；而has\_incomplete\_scratchpad扮演着“质检员”的角色，通过检测推理标签的完整性来识别并过滤因截断导致的数据残缺，从而从源头上保障进入训练环节的数据都是结构完整、逻辑连贯的高质量样本。
...
*   零推理过滤的质量控制：通过\_extract\_reasoning\_stats来统计<REASONING\_SCRATCHPAD> 和reasoning字段的出现次数，如果两者都为零的话，那这条样本被丢弃。为什么呢？ 因为，如果 Agent 在整个对话中完全没有进行显式推理，说明它可能只是在机械地执行操作，没有“思考过程”。这样的样本对训练是没有价值。
